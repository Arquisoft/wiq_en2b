...
public class AuthController {
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "400", description = "Invalid email format or credentials (username or email) already in use"),
            @ApiResponse(responseCode = "401", description = "Invalid email or password, check for them to be correct"),
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Successfully retrieved"),
            @ApiResponse(responseCode = "403", description = "You are not logged in, so there's no log out to do"),
    })
    @GetMapping("/logout")
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Successfully retrieved"),
            @ApiResponse(responseCode = "403", description = "You are not logged in, so there's no token to refresh"),
    })
